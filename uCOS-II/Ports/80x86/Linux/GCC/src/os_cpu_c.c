...
BOOLEAN virtualInterruptFlag=1;
INT32S eventType = 0;
...
BOOLEAN idleTrigger = 1;					//Trigger a message, when the idle task is
...
void OSEnableInterruptFlag(void)
{
    ...
    if (virtualInterruptFlag==0)				//If the timer interrupt previously was disabled,
...
{
    if (virtualInterruptFlag==1)				//If the timer interrupt previously was enabled,
    {   
